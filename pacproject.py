...
def search():
    package_name = request.form['q']
    # package_name = request.args.get('q')
    packages = mongo.db.packages
    existing_package = packages.find({'name' : { '$regex' : ".*" + package_name + '*' } }) # it is basically a cursor object which can be traversed by index
    print(existing_package.count())
    if existing_package.count() == 0:
        return "could not find a package"
    else:
        package_list = list()
        for one_package in existing_package:
            package_list.append(str(one_package['name']))
        print(package_list)
        # return render_template('search_response.html', package_list = package_list)
        return jsonify(package_list=package_list)
        # return jsonify(  {'package_names' : package_list }   )
